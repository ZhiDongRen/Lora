...
{   			    
	u8 temp,t,t1;
	u16 y0=y;				   
    for(t=0;t<192;t++)//每个32*48的汉字点阵 有192个字节
    {  
		if(t<32) temp=asc2_3216[index*6][t];           //前24个字节
		else if(t<64)temp=asc2_3216[index*6+1][t-32]; //中24个字节
        ...
        else temp=asc2_3216[index*6+5][t-160];         //后24个字节
	    for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)LCD_Draw_Point(x,y,color);//画实心点
			else LCD_Draw_Point(x,y,BACK_COLOR);   //画空白点（使用背景色）
...
{   			    
	u16 temp,t,t1;
	u16 y0=y;				   
    for(t=0;t<512;t++)//每个32*32的汉字点阵 有128个字节
    {   
		if(t<64)temp=tfont64[index*8][t];           //前24个字节
		else if(t<128)temp=tfont64[index*8+1][t-64]; //中24个字节
        ...
        else temp=tfont64[index*4+7][t-448];         //后24个字节
	    for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)LCD_Draw_Point(x,y,color);//画实心点
			else LCD_Draw_Point(x,y,BACK_COLOR);   //画空白点（使用背景色）
